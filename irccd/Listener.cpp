...
void Listener::clientRead(Socket &client)
{
	...
	try {
		...
		if (length == 0)
			...
			if (m_clients[client].isFinished(data, ret))
				execute(ret, client);
		}
...
void Listener::execute(const std::string &cmd, Socket &s)
{
	...
	if (cmdDelim != std::string::npos) {
		...
		if (handlers.find(cmdName) == handlers.end())
			...
			notifySocket(result, s);
		}
	}
}

void Listener::notifySocket(const std::string &message, Socket &s)
{
...
void Listener::process()
{
	try {
		...
		auto i = std::find(m_servers.begin(), m_servers.end(), s);
