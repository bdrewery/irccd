...
local function reload(s, c)
	...
	for i = 1, 6 do
		instances[s:info().name][c].cylinder[i] = false
	end

	instances[s:info().name][c].cylinder[math.random(1, 6)] = true
	instances[s:info().name][c].count = 1
end
...
local function init(s, c)
	if instances[s:info().name] == nil then
		instances[s:info().name] = { }
	end

	if instances[s:info().name][c] == nil then
		instances[s:info().name][c] = { }
		reload(s, c)
	end
end
...
local function shot(s, c, who)
	...
	local keywords = {
		...
		u = who,
		i = instances[s:info().name][c].count
	}

	-- Get the instances
	local i = instances[s:info().name][c]
